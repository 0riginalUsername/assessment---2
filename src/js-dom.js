...
const buttonAlert = document.querySelector(`#submit`)

const Alert = () => {
    const Entry = document.querySelector(`#alert-message`).value
    alert(`${Entry}`)
}
...
const buttonAdd = document.querySelector(`#item-adder`)
const addList = () =>
...
buttonAdd.addEventListener(`dblclick`, addList)
// Change colors
//
...
let targetElement = document.getElementsByClassName("changes-colors")

let changeColor = (colorClass) => {
    targetElement.className = colorClass;
}
btnBlue.addEventListener(`click`, () => {
    changeColor(`changesBlue`)
    console.log(`working`);
})
...
const calcBtn = document.querySelector("#factorial-calculator")
const facEntry = document.querySelector("#factorial-input")
...
function answer(){
let Entry = parseInt(document.querySelector(`#factorial-input`).value)
  result = factorial(Entry)
  return resulting.innerText = result
  }
calcBtn.addEventListener(`click`, answer)
console.log(answer());
// Validate a form
//
